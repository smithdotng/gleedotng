...
import { activatePlan, getOperator, getPayment, markPaymentFailed, markPaymentPaid } from "@/lib/store";
...
export async function GET(req: Request) {
  ...
  const dash = `/dashboard/${payment.operatorSlug}`;
  if (status !== "successful" && status !== "completed") {
    await markPaymentFailed(txRef);
    return back(`${dash}/upgrade?payment=cancelled`);
  }

  const result = await verifyPayment(transactionId, { txRef, amount: payment.amount });
  if (!result.ok) {
    await markPaymentFailed(txRef);
    return back(`${dash}/upgrade?payment=failed`);
  }
